...
{{-- HERO --}}
<section class="min-h-[calc(100vh-5rem)] flex items-center bg-gradient-to-br from-blue-50 via-white to-indigo-50">
    <div class="max-w-6xl mx-auto px-4 sm:px-6 py-16 md:py-20 flex flex-col-reverse md:flex-row items-center gap-10 md:gap-16">
        {{-- Text --}}
        <div class="flex-1 text-center md:text-left">
            ...
            <p class="text-lg sm:text-xl text-gray-500 mb-6 font-medium">{{ $profile?->tagline ?? 'Full Stack Developer' }}</p>
            <p class="text-gray-600 leading-relaxed mb-8 max-w-lg mx-auto md:mx-0">
            ...
            <div class="flex flex-wrap gap-3 sm:gap-4 justify-center md:justify-start">
                <a href="{{ route('projects') }}"
                   class="px-5 sm:px-6 py-3 bg-blue-700 hover:bg-blue-800 text-white rounded-lg font-semibold shadow-md transition">
                    Lihat Projects
                </a>
                <a href="{{ route('contact') }}"
                   class="px-5 sm:px-6 py-3 border-2 border-blue-700 text-blue-700 hover:bg-blue-50 rounded-lg font-semibold transition">
                    Hubungi Saya
                </a>
                @if($profile?->github)
                <a href="{{ $profile->github }}" target="_blank"
                   class="px-5 sm:px-6 py-3 border border-gray-300 text-gray-600 hover:bg-gray-50 rounded-lg font-semibold transition">
                    GitHub
                </a>
                @endif
            </div>
        ...
        <div class="shrink-0">
            ...
            <img src="{{ asset('storage/' . $profile->avatar) }}"
                 class="w-36 h-36 sm:w-44 sm:h-44 md:w-56 md:h-56 rounded-2xl object-cover shadow-xl border-4 border-white" alt="Avatar">
            @else
            <div class="w-36 h-36 sm:w-44 sm:h-44 md:w-56 md:h-56 rounded-2xl bg-gradient-to-br from-blue-100 to-indigo-200 flex items-center justify-center shadow-xl">
...
<section class="max-w-6xl mx-auto px-4 sm:px-6 py-16 md:py-20">
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-10">
        <div>
            <h2 class="text-2xl sm:text-3xl font-bold text-gray-900">Featured Projects</h2>
            <p class="text-gray-500 mt-1">Beberapa project pilihan yang pernah saya kerjakan.</p>
        </div>
        <a href="{{ route('projects') }}" class="text-blue-700 font-semibold hover:underline text-sm shrink-0">
            Lihat Semua →
        </a>
    </div>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
        ...
        @empty
        <p class="text-gray-400 sm:col-span-2 lg:col-span-3 text-center py-10">Belum ada project.</p>
        @endforelse
    </div>
</section>

{{-- STATS / ABOUT STRIP --}}
<section class="bg-blue-700 text-white py-14 md:py-16">
    <div class="max-w-6xl mx-auto px-4 sm:px-6 grid grid-cols-2 md:grid-cols-4 gap-6 sm:gap-8 text-center">
        <div>
